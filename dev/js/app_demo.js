...
var app_demo = {
    ...
    googlemap: function(){
        ...
        if($("#google_map_wide").length > 0){
            ...
            var icon = 'assets/images/real-estate/map-marker.png';
            var content = '<div class="listing-large-box">'+
                                    '<div class="row">'+
                                        '<div class="col-xs-4">'+
                                            '<img src="assets/images/real-estate/img-1.jpg" class="img-responsive img-thumbnail mCS_img_loaded">'+
                                        '</div>'+
                                        '<div class="col-xs-8 padding-left-20">'+
                                            '<h5 class="margin-bottom-0 margin-top-5"><a href="#" class="text-default">Great House, Near Lake</a></h5>'+
                                            '<p class="text-muted margin-top-0 margin-bottom-20"><span class="fa fa-map-marker"></span> Velosovo, Korolevskoe</p>'+
                                            '<div class="row margin-bottom-10">'+
                                                '<div class="col-xs-6">'+
                                                    '<span class="text-lg text-blue-500 text-bolder">$112,000</span>'+
                                                    '<p class="text-muted margin-0 text-sm text-bold">$922/mo</p>'+
                                                '</div>'+
                                                '<div class="col-xs-6 text-right">'+
                                                    '<span class="text-lg text-bold">140m2</span>'+
                                                    '<p class="text-muted margin-0 text-sm text-bold">3 rooms, 2011</p>'+
                                                '</div>'+
                                            '</div>'+
                                        '</div>'+
                                    '</div>'+
            ...
            var mapWide = [{"featureType": "administrative.country","elementType": "geometry.fill","stylers": [{"visibility": "on"}]}];

            var gMapWideCords = new google.maps.LatLng(50.43, 30.60);
            var gMapWideOptions = {styles: mapWide,zoom: 8, disableDefaultUI: true, center: gMapWideCords, mapTypeId: google.maps.MapTypeId.ROADMAP};
            var gMapWide = new google.maps.Map(document.getElementById("google_map_wide"), gMapWideOptions);
            ...
            var cords = new google.maps.LatLng(50.37, 30.65);
            var marker = new google.maps.Marker({position: cords, map: gMapWide, title: "Marker 1", icon: icon});

            var cords = new google.maps.LatLng(50.5, 30.55);
            var marker1 = new google.maps.Marker({position: cords, map: gMapWide, title: "Marker 2", icon: icon});

            var cords = new google.maps.LatLng(50.8, 30.55);
            var marker2 = new google.maps.Marker({position: cords, map: gMapWide, title: "Marker 3", icon: icon});

            infowindow.open(gMapWide, marker);

        }
        if($("#google_contact").length > 0){
            var icon = 'assets/images/real-estate/map-marker.png';
            var mapStyle = [{"featureType": "administrative.country","elementType": "geometry.fill","stylers": [{"visibility": "on"}]}];
            var gMapContactCords = new google.maps.LatLng(50.43, 30.55);
            var gMapContactOptions = {styles: mapWide,zoom: 15, disableDefaultUI: true, center: gMapContactCords, mapTypeId: google.maps.MapTypeId.ROADMAP};
            var gMapContact = new google.maps.Map(document.getElementById("google_contact"), gMapContactOptions);

            var cords = new google.maps.LatLng(50.43, 30.55);
            var marker2 = new google.maps.Marker({position: cords, map: gMapContact, title: "Marker 3", icon: icon});
        }
